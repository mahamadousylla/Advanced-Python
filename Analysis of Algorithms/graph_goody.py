...
def random_graph (n : int, edges_to_create: callable) -> Graph:
    ...
    for _i in range(int(edges_to_create(n))):
        n1 = str(randrange(n))
        n2 = str(randrange(n))
        if n1 != n2 and not (n1,n2) in graph:
            v = randint(1,n)
            graph[n1,n2] = v
            graph[n2,n1] = v
    return graph
...
def spanning_tree(graph : Graph) -> {(str,str,int)}:
    answer = set()
    edges = sorted( (e[2],e[0],e[1]) for e in graph if len(e) != 1) 
    components = EquivalenceClass(graph.all_nodes())
    n = len(graph.all_nodes())
    for v,o,d in edges:
        if n == 1:
            return answer
